...
class register {
	...
	function verify_if_email_exists($type = null) {
		...
		if($type) {
			...
			$query = sprintf("SELECT `email` FROM `users`, `suspended` WHERE `email` = '%s' AND `suspended` != 2", $this->db->real_escape_string(mb_strtolower($this->email)));
		}
		$result = $this->db->query($query);
	...
	function validate_values() {
		...
		if (strlen($this->password) < 6 || strlen($this->password) > 30 || preg_match($password_regex, $this->password) == 0) {
	...
	function getTracks($query, $type, $typeVal) {
		...
		foreach($rows as $row) {
			...
			if($hide == 1 && !$this->is_admin) {
				...
				$track .= '<div id="track'.$row['id'].'" class="song-container'.(($type == 'trackPage') ? ' song-container-page' : '').(($row['top']) ? ' top-track' : '').'">
						'.(($row['top']) ? '<div class="top-number">'.$i.'</div>' : '').'
